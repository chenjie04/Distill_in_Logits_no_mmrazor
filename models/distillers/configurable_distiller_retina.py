...
class ConfigurableDistillerInLogits(BaseDistiller):
    ...
    def __init__(
        ...
        # self.student_projector = MODELS.build(self.projector_cfg)

        # for param in self.student_projector.parameters():
        #     param.requires_grad = False
        # self.cls_loss_module = ChannelWiseDivergence()
        # self.cls_loss_module = KLDivergence()
    ...
    def compute_distill_losses(
        ...
        # student_gt_loss = self.teacher_projector.loss_by_feat(
        #     cls_scores_student,
        #     bbox_preds_student,
        #     batch_gt_instances,
        #     batch_img_metas,
        ...
        # for key, value in student_gt_loss.items():
        #     losses[f"student.{key}"] = torch.sum(torch.stack(value))
        ...
        for cls_score_s, cls_score_t in zip(
            ...
            # Normalization很重要，会严重影响性能
            # v3再测试一次, Normalization很重要
            cls_score_s = self.layer_norm(cls_score_s)
            cls_score_t = self.layer_norm(cls_score_t.detach())
            # cls_loss += F.smooth_l1_loss(cls_score_s, cls_score_t, reduction="mean")
            # cls_l1_loss += F.smooth_l1_loss(cls_score_s, cls_score_t, reduction="mean")
            cls_score_s = (
