...
from models.ops_dcnv3.functions import DCNv3Function
from models.ops_dcnv3.modules.dcnv3 import (
    to_channels_first,
    to_channels_last,
    _is_power_of_2,
)
...
class FeadForward(nn.Module):
    def __init__(
        ...
        self.output_project = nn.Conv2d(
            self.hidden_fetures, self.in_channel, kernel_size=1, bias=True
        )  # 1x1 conv
...
class GroupDeformableConvModule(nn.Module):
    def __init__(
        ...
        mlp_ratio: float = 1.0,
        drop_rate: float = 0.1,
        norm_cfg: ConfigType = dict(type="GN", num_groups=4, requires_grad=True),
        act_cfg: ConfigType = dict(type="GELU"),
    ) -> None:
        super().__init__()
        # # Normalization:
        ...
        self.feed_forward = FeadForward(
            channels, mlp_ratio=mlp_ratio, drop_rate=drop_rate, groups=groups, act_cfg=act_cfg
        )
